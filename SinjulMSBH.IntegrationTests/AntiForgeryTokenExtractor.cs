...
namespace SinjulMSBH.IntegrationTests
{
    public static class AntiForgeryTokenExtractor
    {
        ...
        {
            string cookie = ExtractAntiForgeryCookieValueFrom(response);
            string token = ExtractAntiForgeryToken(await response.Content.ReadAsStringAsync());
        ...
        {
            string antiForgeryCookie = response.Headers.GetValues("Set-Cookie").FirstOrDefault(x => x.Contains(AntiForgeryCookieName));
        ...
        {
            Match requestVerificationTokenMatch =
